...
class Authenticate:
    ...
    def __init__(
        ...
        self.t = Translator("locale", language).translate
    ...
    def login(self, form_name: str, location: str = "main") -> tuple:
        ...
        if location not in ["main", "sidebar"]:
            raise ValueError(self.t("error_location_main_sidebar"))
        if not st.session_state["authentication_status"]:
            self._check_cookie()
            if not st.session_state["authentication_status"]:
                if location == "main":
                    login_form = st.form(self.t("login_title"))
                elif location == "sidebar":
                    login_form = st.sidebar.form(self.t("login_title"))

                login_form.subheader(form_name)
                self.username = login_form.text_input(self.t("username")).lower()
                st.session_state["username"] = self.username
                self.password = login_form.text_input(
                    self.t("password"), type="password"
                )

                if login_form.form_submit_button(self.t("login_submit")):
    ...
    def reset_password(
        ...
        if location not in ["main", "sidebar"]:
            raise ValueError(self.t("error_location_main_sidebar"))
        if location == "main":
            reset_password_form = st.form(self.t("reset_password"))
        elif location == "sidebar":
            reset_password_form = st.sidebar.form(self.t("reset_password"))

        reset_password_form.subheader(form_name)
        self.username = username.lower()
        self.password = reset_password_form.text_input(
            self.t("current_password"), type="password"
        )
        new_password = reset_password_form.text_input(
            self.t("new_password"), type="password"
        )
        new_password_repeat = reset_password_form.text_input(
            self.t("repeat_password"), type="password"
        )
                ...
                if len(new_password) > 0:
                    if new_password == new_password_repeat:
                        if self.password != new_password:
                            ...
                            raise ResetError(self.t("error_same_password"))
                    else:
                        raise ResetError(self.t("error_passwords_not_match"))
                else:
                    raise ResetError(self.t("error_no_new_password"))
            else:
    ...
    def _register_credentials(
        ...
        if not self.validator.validate_username(username):
            raise RegisterError(self.t("error_username_not_valid"))
        if not self.validator.validate_name(name):
            raise RegisterError(self.t("error_name_not_valid"))
        if not self.validator.validate_email(email):
            raise RegisterError(self.t("error_email_not_valid"))
    ...
    def register_user(
        ...
        if preauthorization:
            if not self.preauthorized:
                raise ValueError(self.t("error_preauthorization_not_none"))
        if location not in ["main", "sidebar"]:
            raise ValueError(self.t("error_location_main_sidebar"))
        if location == "main":
            register_user_form = st.form(self.t("register_title"))
        elif location == "sidebar":
            register_user_form = st.sidebar.form(self.t("register_title"))

        register_user_form.subheader(form_name)
        new_email = register_user_form.text_input(self.t("email"))
        new_username = register_user_form.text_input(self.t("username")).lower()
        new_name = register_user_form.text_input(self.t("name"))
        ...
        if register_user_form.form_submit_button(self.t("register_button")):
            if (
                len(new_email)
                    ...
                    if new_password == new_password_repeat:
                        if preauthorization:
                            if new_email in self.preauthorized["emails"]:
                                ...
                                raise RegisterError(
                                    self.t("error_user_not_preauthorized")
                                )
                        else:
                            self._register_credentials(
                                new_username,
                        ...
                        raise RegisterError(self.t("error_passwords_not_match"))
                else:
                    raise RegisterError(self.t("error_username_not_available"))
            else:
                raise RegisterError(self.t("error_missing_fields"))
    ...
    def forgot_password(self, form_name: str, location: str = "main") -> tuple:
        ...
        if location not in ["main", "sidebar"]:
            raise ValueError(self.t("error_location_main_sidebar"))
        if location == "main":
            forgot_password_form = st.form(self.t("forgot_password"))
        elif location == "sidebar":
            forgot_password_form = st.sidebar.form(self.t("forgot_password"))

        forgot_password_form.subheader(form_name)
        username = forgot_password_form.text_input(self.t("username")).lower()

        if forgot_password_form.form_submit_button(self.t("reset_submit")):
            if len(username) > 0:
                ...
                raise ForgotError(self.t("error_username_not_provided"))
    ...
    def forgot_username(self, form_name: str, location: str = "main") -> tuple:
        ...
        if location not in ["main", "sidebar"]:
            raise ValueError(self.t("error_location_main_sidebar"))
        if location == "main":
            forgot_username_form = st.form(self.t("forgot_username_title"))
        elif location == "sidebar":
            forgot_username_form = st.sidebar.form(self.t("forgot_username_title"))

        forgot_username_form.subheader(form_name)
        email = forgot_username_form.text_input(self.t("email"))

        if forgot_username_form.form_submit_button(self.t("submit")):
            if len(email) > 0:
                return self._get_username("email", email), email
            else:
                raise ForgotError(self.t("error_email_not_provided"))
        return None, email
    ...
    def update_user_details(
        ...
        if location not in ["main", "sidebar"]:
            raise ValueError(self.t("error_location_main_sidebar"))
        if location == "main":
            update_user_details_form = st.form(self.t("update_user_title"))
        elif location == "sidebar":
            update_user_details_form = st.sidebar.form(self.t("update_user_title"))

        update_user_details_form.subheader(form_name)
        self.username = username.lower()
        field = update_user_details_form.selectbox("Field", ["Name", "Email"]).lower()
        new_value = update_user_details_form.text_input(self.t("new_value"))

        if update_user_details_form.form_submit_button(self.t("submit")):
            if len(new_value) > 0:
                if new_value != self.credentials["usernames"][self.username][field]:
                    ...
                    raise UpdateError(self.t("error_values_are_the_same"))
            if len(new_value) == 0:
                raise UpdateError(self.t("error_new_value_not_provided"))
